...
class ParserVideo
{
    ...
    static public function parse($url=''){
		...
		$extra = array();
		$ourl = $url;
		$goali = isset($_GET['cache']);
		$lockSourceList = array(
			1966045 => array("t"=>"youku","i"=>"XODk5MTIyNjE2")
		);
        $xnfo = explode('|',$url);
		if($url>0||count($xnfo)==3){
            if(count($xnfo)==3){
				$ourl = $xnfo[0];
                $_info = array("success"=>true,"sourceType"=>$xnfo[1],"sourceId"=>$xnfo[2]);
            }else{
                $vvid = intval($url);
                if($lockSourceList[$vvid]){
                    $_info = array("success"=>true,"sourceType"=>$lockSourceList[$vvid]["t"],"sourceId"=>$lockSourceList[$vvid]["i"]);
                }else{
                    $_info = json_decode(Base::_cget("http://www.acfun.tv/video/getVideo.aspx?id=".$url),true);
                }
            }
			if($_info["success"]){
				$_info["sourceType"] = str_ireplace("2","",$_info["sourceType"]);
				if($_info["sourceType"]=="youku"){
					$url = "http://v.youku.com/v_show/id_".$_info["sourceId"].".html";
				}elseif($_info["sourceType"]=="sina"){
					$url = "http://video.sina.com.cn/v/b/".$_info["sourceId"]."-0.html";
				}elseif($_info["sourceType"]=="iqiyi"){
					$url = "http://www.iqiyi.com/vid_".$_info["sourceId"].".html";
				}elseif($_info["sourceType"]=="sohu"){
					$url = "http://tv.sohu.com/?vid=".$_info["sourceId"];
				}else{
					$goali = true;
				}
			}else{
		...
		if($goali){
			if(count($xnfo)>1){
				$data = array("code"=>500);
				$i = 0;
				while($data['code']!=200 && $i<5){
					$data = json_decode(Base::_cget("http://acfnic2.aliapp.com/index.php?vid=".$ourl."&type=mobileclient"),true);
					$i++;
				}
				$data = $data['result'];
				if($data['C00']){
					$data['C00']['try_times'] = $i;
				}
			}else{
				$date = false;
			}
		...
		if(!$data || count($data)==0){header("Location:http://acfnic2.aliapp.com/index.php?vid=".$ourl."&type=mobileclient");exit;}
        return $data;
    }
